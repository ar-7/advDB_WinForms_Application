...
{
    public partial class HotelRoomTable : Form
    {
        ...
        {
            this.Validate();
            this.hotel_roomBindingSource.EndEdit();
            this.tableAdapterManager.UpdateAll(this.hotel_bookingDataSet);
            this.hotel_roomTableAdapter.Fill(this.hotel_bookingDataSet.hotel_room); // Refresh the data grid view, fixes the negative id display
        }
        // Function that runs when the form is closed.
        private void HotelTable_FormClosed(object sender, FormClosedEventArgs e)
        {
            hotel_roomDataGridView.DataSource = null; // This is a fix to automatic API behaviour that causes an exception when the user closes the form.
        ...
        private void hotel_roomDataGridView_RowValidating(object sender, DataGridViewCellCancelEventArgs e)
        {
            if (hotel_roomDataGridView.IsCurrentRowDirty)
            {
                if (!String.IsNullOrEmpty(hotel_roomDataGridView.Rows[e.RowIndex].ErrorText))
                {
                    e.Cancel = true;
                }
            }
        }
        private void hotel_roomDataGridView_DataError(object sender, DataGridViewDataErrorEventArgs e)
        {
            hotel_roomDataGridView.Rows[e.RowIndex].ErrorText = e.Exception.Message + "Please complete content or delete row to continue.";
            e.Cancel = true;
        }
        private void hotel_roomDataGridView_CellValidated(object sender, DataGridViewCellEventArgs e)
        {
            if (hotel_roomDataGridView.IsCurrentRowDirty)
            {
                if (!String.IsNullOrEmpty(hotel_roomDataGridView.Rows[e.RowIndex].ErrorText))
                {
                    hotel_roomDataGridView.Rows[e.RowIndex].ErrorText = String.Empty;
                }
            }
        } // ***
